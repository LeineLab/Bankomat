    ...
    def parse(self):
        ...
        while self.ser.in_waiting > 0:
            ...
            if b == self.BUSY:
                ...
                self._recentlyBusy()
                and b >= self.NOTE_ACCEPT1
                and b <= self.NOTE_ACCEPT16
            ):
                ...
                if self._escrowEnabled:
                    ...
                    self.logger.info(
                        "Accepted value currently at %d" % self._acceptedVal
                    )
                self._last_busy = 0  # Reset busy state, as note is processed
            elif (
                b == self.REJECT_ESCROW
                or b == self.ESCROW_ABORT
                or b == self.NOTE_TAKEN
            ):
                self._escrowVal = 0
                ...
                self._escrowVal = 0
                self.logger.info(
                    "Escrow accepted, setting to zero, accepted value at %d"
                    % self._acceptedVal
                )
        ...
        try:
            ...
            for i in range(16):
                if i < len(self._enabled_notes) and self._enabled_notes[i]:
                    self.logger.debug(
                        "Enable channel %d / %d Euro" % (i, self._notes[i])
                    )
                    self.send(self.NOTE_ENABLE1 + i)
                else:
                    self.logger.debug(
                        "Disable channel %d / %d Euro"
                        % (i, self.getBillValue(self.NOTE_ACCEPT1 + i))
                    )
